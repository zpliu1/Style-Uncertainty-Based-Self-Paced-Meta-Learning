    ...
    def forward(self, inputs, targets):
        ...
        if self.use_semi:
            ...
            for i in range(n):
                ...
                #dis_ap_al.append(dist[i][mask[i]].mean())
                #dis_an_al.append(dist[i][mask[i] == 0].mean())
                dis_ap_al = dis_ap_al + dist[i][mask[i]].mean()
                dis_an_al = dis_an_al + dist[i][mask[i] == 0].mean()

        dis_ap_al = dis_ap_al/64
        dis_an_al = dis_an_al/64

        dis_ap_al_c = dis_ap_al.clone().detach()
        dis_an_al_c = dis_an_al.clone().detach()

        #dis_ap_m = np.mean(np.array(dis_ap_al),axis=0)
        #dis_an_m = np.mean(np.array(dis_an_al),axis=0)
