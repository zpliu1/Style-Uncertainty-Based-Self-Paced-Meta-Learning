...
class MetaResNetBase(MetaModule):
    def __init__(self, layers, block=Bottleneck):
        ...
        self.mixstyle = MixStyle(p=0.5, alpha=0.3, mix='crossdomain')
    ...
    def forward(self, x, MTE=False, index=10):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.maxpool(x)
...
class MetaResNet(MetaModule):
    def __init_with_imagenet(self, baseModel):
        ...
        model.load_state_dict(torch.load('/home/zhang/E/lzp/DG/M3L/M3L-master/reid/models/resnet50-19c8e357.pth'))
        del model.fc
    ...
    def __init__(self, num_features=0, dropout=0, cut_at_pooling=False, norm=True, num_classes=[0,0,0], BNNeck=False):
        ...
        #self.feat_bn_max = MixUpBatchNorm1d(self.num_features)
        #init.constant_(self.feat_bn_max.weight, 1)
        #init.constant_(self.feat_bn_max.bias, 0)

    def forward(self, x, MTE='', save_index=0, index=10):
        inin = index

        x= self.base(x, index=inin)
        ...
        if self.has_embedding:
            ...
            #bn_x_max = self.feat_bn_max(x_max, MTE, save_index)
        tri_features = x

        if self.training is False:
            #bn_x_max = F.normalize(bn_x_max)
